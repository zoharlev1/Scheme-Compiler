...
%macro MAKE_LITERAL 2
 ...
 %define MAKE_LITERAL_BOOL(value) MAKE_LITERAL T_BOOL,db value
 %define MAKE_LITERAL_CHAR(value) MAKE_LITERAL T_CHAR, db value
 %define MAKE_LITERAL_FLOAT(value) MAKE_LITERAL T_FLOAT, dq value
...
%macro ENV_EXT 1
 	...
 	%%COPY_ARGS:
 		cmp rdx,%1							
 		je %%END_COPY_ARGS
 		mov rcx,[rbp + WORD_SIZE *2]				
 		shl rdx, 3
 		add rcx,rdx						
 		shr rdx,3
 		mov rcx,[rcx]						
 		mov [rax+ WORD_SIZE*rbx],rcx 					
		inc rdx									
 		inc rbx									
 		jmp %%COPY_ARGS
 	...
 	%%PARAMS_LOOP:
 		cmp rdx,[rbp+ WORD_SIZE*3]						
 		jae %%END_PARAMS_LOOP
 		mov rcx,[rbp + WORD_SIZE*(4+rdx)]			
 		mov rbx,[rax]						
 		mov [rbx+(WORD_SIZE*rdx)],rcx					
 		inc rdx								
 		jmp %%PARAMS_LOOP
...
%endmacro
;----------------------------------
   %macro FIX_LAMBDAOPT 1
 	mov rax, %1
 	ARGC_RPS rbx
	cmp [rsp + WORD_SIZE*2], rax						
 	jb %%MAGIC_LOOP
		...
		%%CREATE_PROPER_LIST_LOOP:
			cmp rbx, rdx						
			jnbe %%END_CREATE_PROPER_LIST_LOOP
			ARGC_RPS rdi				
			add rdi, 2							
			sub rdi, rbx							
			mov rdi, [rsp + WORD_SIZE*rdi]				
			MAKE_PAIR(rax,rdi,rcx)
			mov rcx, rax
			inc rbx
			jmp %%CREATE_PROPER_LIST_LOOP

		%%END_CREATE_PROPER_LIST_LOOP:
		mov rbx, 0 		
		mov [rsp + WORD_SIZE*(2+%1)], rcx					
		...
		%%STACK_DOWN:
			cmp rbx, rdx
			jnb %%END_STACK_DOWN
			mov rax,[ rsp + WORD_SIZE*(rbx+1)]					
			mov [rsp + WORD_SIZE*rbx], rax					
			inc rbx
			jmp %%STACK_DOWN

		%%END_STACK_DOWN:
		mov rax,SOB_NIL_ADDRESS
		mov [rsp + WORD_SIZE*(2+%1)], rax				
		mov rax, %1
		mov [rsp + WORD_SIZE*2],rax	
		jmp %%END_MAGIC_LOOP
